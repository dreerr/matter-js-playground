...
export async function loadSVGPaths(city) {
...
export function parsePath(pathData, scale, offsetX, offsetY) {
  ...
  const vertices = parsed
    .filter((segment) => segment.code === "M" || segment.code === "L")
    .filter((segment) => {
      if (segment.code === "M") mCodeCount++;
      return mCodeCount < 2;
    })
    .map((segment) => ({
      x: (segment.x - offsetX) * scale,
      y: (segment.y - offsetY) * scale,
    }));
  const simplification = Math.min(1.2, 0.002 * vertices.length);
  return simplify(vertices, simplification, false);
}
